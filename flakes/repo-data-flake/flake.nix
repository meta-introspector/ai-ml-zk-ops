    ...
    flake-utils-wrapper.lib.eachDefaultSystem (system:
      let
        ...
        repo2gramJson = pkgs.runCommand "repo-2gram-json" {
          ...
          convertJsonScript = "${self.src}/source/automation/convert_2gram_csv_to_json.sh";
        ...
        repoAttrs = builtins.listToAttrs (map (item: { name = "${item.owner}-${item.repo}"; value = mkRepoAttr item; })
          repoData);
      in
      {
