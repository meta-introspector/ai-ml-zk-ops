...
if [ -z "$FLAKE_NIX_FILE" ] || [ -z "$OUTPUT_FILE" ]; then
  echo "Usage: $SCRIPT_NAME <flake.nix_file> <output.verbose-log.txt_file>"
  exit 1
fi

echo "Dumping verbose log for ${FLAKE_NIX_FILE}"

FLAKE_DIR=$(dirname "$FLAKE_NIX_FILE")

# Run the nix command and capture its output and exit code
NIX_OUTPUT=$(cd "$FLAKE_DIR" && nix -v flake show . 2>&1)
NIX_EXIT_CODE=$?
